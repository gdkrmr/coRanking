...
image(log(t(apply(Q.tsne,2,rev))),
      main = 't-SNE', axes = F)
axis(2, at = c(0,1), labels = c(nrow(data)-1, 1))
axis(3, at = c(0,1), labels = c(1, nrow(data)-1))

image(log(t(apply(Q.pca,2,rev))),
      main = 'PCA', axes = F)
axis(2, at = c(0,1), labels = c(nrow(data)-1, 1))
axis(3, at = c(0,1), labels = c(1, nrow(data)-1))
```	
...
Defined by **TODO:CITE**. A general quality measure depending on $K$
could be defined as:

$$\text{LCMC}(K) = \frac {1}{KN} \sum_{i,j\in {1, \ldots, K} - \frac{K}{N-1}}
q_{i,j}$$

```{r, fig.show='hold', fig.width=4, fig.height=4}
lcmc.tsne <- numeric(nrow(Q.tsne))
lcmc.pca <- numeric(nrow(Q.tsne))
for(i in 1:nrow(Q.tsne)){
    lcmc.tsne[i] <- coRanking:::cm.LCMC(Q.tsne, i)
    lcmc.pca[i] <- coRanking:::cm.LCMC(Q.pca, i)
}
