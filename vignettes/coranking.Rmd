...
```{r, fig.show="hold", fig.width = 7, fig.height = 7, out.width = "45%"}
qnx.tsne <- Q_NX(Q.tsne)
qnx.pca <- Q_NX(Q.pca)
Kmax.tsne <- which.max(qnx.tsne)
Kmax.pca <- which.max(qnx.pca)
plot(qnx.tsne, main = "t-SNE", xlab = "K", ylab = expression(Q[NX]), type = "l")
plot(qnx.pca, main = "PCA", xlab = "K", ylab = expression(Q[NX]), type = "l")
```
